...
class Downsampler(nn.Module):
    ...
    def __init__(self, n_planes, factor, kernel_type, phase=0, kernel_width=None, support=None, sigma=None, preserve_size=False):
        ...
        downsampler = nn.Conv2d(n_planes, 1, kernel_size=self.kernel.shape, stride=factor, padding=0)
        downsampler.weight.data[:] = 0  # shape (n_planes, n_planes, 8, 8)
        downsampler.bias.data[:] = 0   # should be zero

        kernel_torch = torch.from_numpy(self.kernel)
        for i in range(n_planes):
            # downsampler.weight.data[i, i] = kernel_torch
            downsampler.weight.data[0, i] = kernel_torch
