...
class QDGP_hybrid(nn.Module):
    def __init__(self, config):
        ...
        if self.measurement_setting == 'd':
            self.QNNPrior = EnsembleQCBM_001(self.N_heads, self.N_qubits, self.N_layers, self.measurement_setting)
        elif self.measurement_setting == 's':
            self.QNNPrior = EnsembleQCBM_single_001(self.N_heads, self.N_qubits, self.N_layers, self.measurement_setting)
        # classical net
...
class QDGP_0001(object):
            ...
            for i in range(iteration):
                ...
                if i == 0 or (i + 1) % self.config['print_interval'] == 0:
                    if self.rank == 0:
                        ...
                        if self.config['random_G']:
                            np.save("./experiments/data_recon/{}/QDGP1280001_noise_mode{}_recons_dim{}_iter{}.npy"
                            ...
                            np.save("./experiments/data_recon/{}/QDGP1280001_noise_mode{}_prior_dim{}_iter{}.npy"
                                    .format(self.object, self.model.measurement_setting, self.dim, curr_step),
                                    qout.detach().cpu().numpy())
                            np.save("./experiments/data_recon/{}/QDGP1280001_noise_mode{}_prior_dim{}_iter{}.npy".format(
                                self.object, self.model.measurement_setting, self.dim, curr_step),
                                qout.detach().cpu().numpy())
                        else:
                            np.save("./experiments/data_recon/{}/QDGP1280001_noise_pretrain_mode{}_recons_dim{}_iter{}.npy"
                                    .format(self.object, self.model.measurement_setting, self.dim,
                                            curr_step),
                                    qout.detach().cpu().numpy())
                            np.save("./experiments/data_recon/{}/QDGP1280001_noise_pretrain_mode{}_prior_dim{}_iter{}.npy".format(
                                self.object, self.model.measurement_setting, self.dim, curr_step),
                                qout.detach().cpu().numpy())
    ...
    def to_img(self, images, iterations, random_G):
        ...
        im = im.convert("L")
        if random_G:
            im.save("./experiments/data_recon/{}/QDGP1280001_noise_mode{}_recons_dim{}_iter{}.jpeg"
                    .format(self.object,
                   self.model.measurement_setting,
                   self.dim,
                   iterations))
        else:
            im.save("./experiments/data_recon/{}/QDGP1280001_noise_pretrain_mode{}_recons_dim{}_iter{}.jpeg"
                    .format(self.object,
                            self.model.measurement_setting,
                            self.dim,
                            iterations))
