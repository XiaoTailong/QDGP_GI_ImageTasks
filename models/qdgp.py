...
class QDGP_hybrid(nn.Module):
    def __init__(self, config):
        ...
        self.factor = 2 if config['resolution'] == 128 else 4  # 64 * 64 image resolution
...
class QDGP_64(object):
            ...
            for i in range(iteration):
                ...
                if i == 0 or (i + 1) % self.config['print_interval'] == 0:
                    if self.rank == 0:
                        ...
                        if self.config['random_G']:
                            np.save("./experiments/data_recon/{}/QDGP_noise_mode{}_recons_dim{}_iter{}.npy"
                            ...
                            np.save("./experiments/data_recon/{}/QDGP_noise_mode{}_prior_dim{}_iter{}.npy".format(
                                self.object, self.model.measurement_setting, self.dim, curr_step),
                                    qout.detach().cpu().numpy())

                        else:
                            np.save("./experiments/data_recon/{}/QDGP_noise_pretrain_mode{}_recons_dim{}_iter{}.npy"
                                    .format(self.object, self.model.measurement_setting, self.dim,
                                            curr_step, False),
                                    np.array(loss_list))
                            np.save("./experiments/data_recon/{}/QDGP_noise_pretrain_mode{}_prior_dim{}_iter{}.npy".format(
                                self.object, self.model.measurement_setting, self.dim, curr_step),
                                qout.detach().cpu().numpy())
    ...
    def to_img(self, images, iterations, random_G):
        ...
        im = im.convert("L")
        if random_G:
            im.save("./experiments/data_recon/{}/QDGP_noise_mode{}_recons_dim{}_iter{}.jpeg"
                    .format(self.object,
                   self.model.measurement_setting,
                   self.dim,
                   iterations))
        else:
            im.save("./experiments/data_recon/{}/QDGP_noise_pretrain_mode{}_recons_dim{}_iter{}.jpeg"
                    .format(self.object,
                            self.model.measurement_setting,
                            self.dim,
                            iterations))
