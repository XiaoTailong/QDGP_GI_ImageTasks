...
def add_example_parser(parser):
    ...
    parser.add_argument('--measurement_setting', type=str, default='s', help='quantum mode')
...
dgp_qp.set_target(img, category, config['image_path'])


loss_dict = dgp_qp.run()
...
if config['dgp_mode'] == 'category_transfer':
    ...
    for i in range(7, 25):  # bird
        ...
        with torch.no_grad():
            x = dgp_qp.model(inputs, i_label, 1.0)
            utils.save_img(
                x[0],
                '%s/images/%s_class%d.jpg' % (config['exp_path'], dgp_qp.img_name, i))
...
############# morphing 不做了
    ...
    weight1 = dgp_qp.model.state_dict()  # 是不是可以全部当模型插入使用
    ...
    with torch.no_grad():
        for i in range(11):
            ...
            # 实际上是embedding
            y_interp = alpha * dgp_qp.model.G.shared(dgp_qp.y) + (1 - alpha) * dgp_qp_2.model.G.shared(dgp_qp_2.y)
            ...
            # save images
            save_path = '%s/images/%s_%s' % (config['exp_path'], dgp_qp.img_name, dgp_qp_2.img_name)
    ...
    vutils.save_image(
        save_imgs,
        '%s/images_sheet/morphing_%s_%s.jpg' % (config['exp_path'], dgp_qp.img_name, dgp_qp_2.img_name),
        nrow=int(save_imgs.size(0) ** 0.5),
        normalize=True)
