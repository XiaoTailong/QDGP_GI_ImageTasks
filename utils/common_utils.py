...
def get_bucket(bucket_path, dims):

    bucket = np.load(bucket_path)[:dims, ]

    return bucket/255.0


def get_bucket_128(bucket_path, dims):

    bucket = np.load(bucket_path)[:dims, ]

    return bucket

def get_pattern(pattern_path, dims):
    # 也是按照需要选取维度
    patterns = np.load(pattern_path)[:dims, ]

    return patterns/255.0


# def get_pattern_128(pattern_path, dims):
#     # 也是按照需要选取维度
#     patterns = np.load(pattern_path)[:dims, ]
#
#     return patterns/255.0/255.0
